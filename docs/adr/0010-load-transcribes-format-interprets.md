...
- **Read-time column pruning cannot recover that on `eit`.** `read_parquet_pruned` resets `columns_to_read` to `None` whenever the cache has no resolvable datetime axis (`io/file_utils.py:568-571`), which is permanently true of EIT's float64 index. Consistent with [ADR-0007](0007-read-time-pruning-is-an-optimization.md) — pruning is an optimization, so declining to prune is always safe — but it means the full cache is also fully read.
