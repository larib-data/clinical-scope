...
1. **Regenerate the user guide PDF.**
   ```bash
...
5. **Publish for real** — undraft the GitHub Release (click *Publish release*). This triggers [`publish-pypi.yml`](../.github/workflows/publish-pypi.yml), which uploads the wheel + sdist to PyPI.
   ```bash
   pip install clinical-scope==X.Y.Z
   ```
   → run `clinical-scope`, check the example.
   → `clinical-scope --demo` — a pip install ships no example, so this prints the link to `clinical-scope-example.zip`. Check it works.
...
**Zenodo / DOI:** the repo is wired to [Zenodo's GitHub integration](https://zenodo.org/account/settings/github/), so publishing the GitHub Release (step 5) auto-archives it and mints a new version DOI under the concept DOI [`10.5281/zenodo.20830140`](https://doi.org/10.5281/zenodo.20830140). The README badge and [`CITATION.cff`](../CITATION.cff) track that concept DOI (always resolves to the latest release), so the DOI itself never needs editing per release.
