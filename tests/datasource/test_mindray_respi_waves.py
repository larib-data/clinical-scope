"""Tests for mindray_respi_waves datasource — waveform block expansion."""
...
def ds_folder(patient_full_path, mindray_respi_waves_cls):
    folder = mindray_respi_waves_cls._find_folder(patient_full_path)
    if folder is None:
        pytest.skip("mindray_respi_waves folder not found in Patient_full")
    return folder
...
class TestFind:
    ...
    def test_find_returns_file(self, ds_folder, mindray_respi_waves_cls):
        result = mindray_respi_waves_cls._find(ds_folder)
...
class TestLoad:
    ...
    def test_load_multiple_columns(self, loaded_df):
        """Waveform expansion should produce multiple columns (one per waveform type)."""
        assert len(loaded_df.columns) > 1

    def test_load_high_row_count(self, loaded_df):
        """Block expansion should produce many more rows than original blocks."""
        assert len(loaded_df) > 100


class TestFormat:
    @pytest.fixture(scope="class")
    def formatted_df(self, loaded_df, patient_options_full, mindray_respi_waves_cls):
        return mindray_respi_waves_cls._format(loaded_df, patient_options_full, {})
...
class TestLoadCsv:
    def test_load_csv(self, patient_difficult_path, mindray_respi_waves_cls):
        folder = mindray_respi_waves_cls._find_folder(patient_difficult_path)
        if folder is None:
            pytest.skip("mindray_respi_waves not in Patient_difficult")
