...
def ds_folder(patient_full_path, mindray_respi_numerics_cls):
    ...
    if folder is None:
        pytest.skip("mindray_respi_numerics folder not found in demo_patient")
    return folder
...
def loaded_df(ds_folder, mindray_respi_numerics_cls):
    ...
    return mindray_respi_numerics_cls._load(file_path)
...
class TestLoadCsv:
    def test_load_csv(self, patient_difficult_path, mindray_respi_numerics_cls):
        ...
        df = mindray_respi_numerics_cls._load(file_path)
