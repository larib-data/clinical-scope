...
# ---------------------------------------------------------------------------
# Low-level: read_parquet_with_datetime_pushdown against a materialized index
# ---------------------------------------------------------------------------
...
class TestReadParquetWithDatetimePushdownLowLevel:
    ...
    def test_one_sided_start_only(self, path, tz):
        ...
        actual = read_parquet_with_datetime_pushdown(path, bounds_fn=lambda _tz: (start, None))
    ...
    def test_one_sided_end_only(self, path, tz):
        ...
        actual = read_parquet_with_datetime_pushdown(path, bounds_fn=lambda _tz: (None, end))
    ...
    def test_no_bounds_returns_full_unfiltered_read(self):
        actual = read_parquet_with_datetime_pushdown(
    ...
    def test_bounds_fn_receives_stored_index_tz(self):
        ...
        read_parquet_with_datetime_pushdown(TZ_AWARE_STORED_INDEX_PARQUET, bounds_fn)
        assert seen_tz == ["Europe/Paris"]

        seen_tz.clear()
        read_parquet_with_datetime_pushdown(TZ_NAIVE_STORED_INDEX_PARQUET, bounds_fn)
        assert seen_tz == [None]
