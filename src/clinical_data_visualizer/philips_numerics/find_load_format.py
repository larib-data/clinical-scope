...
import clinical_data_visualizer.philips_numerics.options as options_naming  # noqa: E501
...
def _format(df: pd.DataFrame, patient_options: dict) -> pd.DataFrame:
    # Apply time-shift
    time_shift_second = patient_options.get(datasource.DataSource.PhilipsNumerics.NAME, {}).get(
...
def main(
    ...
    database_options_numerics = (
        database_options_specific if database_options_specific is not None else {}
    )

    patient_options_numerics = patient_options.get(datasource.DataSource.PhilipsNumerics.NAME, {})
    ...
    list_signal_container = _extract_signals(
        df,
        patient_options=patient_options_numerics,
        database_options_specific=database_options_numerics,
    )
