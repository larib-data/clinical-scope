...
src/clinical_scope/
  ...
  plot_types/
    ...
    registry.py         AVAILABLE schemas, PAGE_ORDER, schema_for(); no plot.py imports
...
- `schema.py` — the config half: `NAME`/`SECTION_KEY`, config keys, `validate()`, `map_refs()`, xlsx sheet + row interpretation, and the six capability flags. Imports nothing but `validation`, so checking a config never loads a plotting library.
